...
class ModalCustom extends Component<{ modalContainerStyle: object, easing: string, duration: number, blurAmount: number, items: { header: Element; content?: Element, onPress?: () => void }[] }> {
	constructor(props) {
		super(props);
		...
		this.state = { item1ContentHeightState: 0 }
	}
	componentDidMount() {
	...
	calcHeadersHeight = () => {
		return this.firstHeaderHeight +
		 this.secondHeaderHeight +
		 this.thirdHeaderHeight
	}
	animate = (ref: string,  height: number, animated: boolean = true) => {
	...
	slideUpContainer = () => {
		...
		this.animate('animRefContainer', this.containerHeightFromChilds || this.item1DefaultContentHeight + this.calcHeadersHeight());
	...
	render() {
		...
		const { item1ContentHeightState } = this.state;
		return (
